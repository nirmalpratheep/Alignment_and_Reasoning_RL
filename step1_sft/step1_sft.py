"""Main orchestrator for dual-GPU SFT training pipeline."""
...
def main():
    """Main orchestrator for dual-GPU training."""
    # Load configuration
    config = load_config("config/sft_config.yaml")
    validate_config(config)
    
    print("\n" + "="*80)
    print("DUAL-GPU SFT TRAINING PIPELINE")
    print("="*80)
    ...
    run_name = f"math-sft-dual-gpu-{timestamp}"
    ...
    # Run training loop on GPU 0 (main process)
    try:
        training_loop(
            config=config,
